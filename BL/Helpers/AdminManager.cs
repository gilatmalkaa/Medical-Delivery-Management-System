...
internal static class AdminManager //stage 4
{
    ...
    internal static void UpdateClock(DateTime newClock) //stage 4-7
    {
        ...
        //StudentManager.PeriodicStudentsUpdates(oldClock, newClock); // in stage 7 and replaced as below
        //TO_DO: //stage 7
    ...
    internal static void SetConfig(BO.Config configuration) //stage 4
    {
        ...
        if (s_dal.Config.FootSpeed != configuration.FootSpeed) //stage 4
        {
            s_dal.Config.FootSpeed = configuration.FootSpeed;
            configChanged = true;
        }
        if (s_dal.Config.BikeSpeed != configuration.BikeSpeed) //stage 4
        {
            s_dal.Config.BikeSpeed = configuration.BikeSpeed;
            configChanged = true;
        }
        if (s_dal.Config.MotorcycleSpeed != configuration.MotorcycleSpeed) //stage 4
        {
            s_dal.Config.MotorcycleSpeed = configuration.MotorcycleSpeed;
            configChanged = true;
        }
        if (s_dal.Config.CarSpeed != configuration.CarSpeed) //stage 4
        {
            s_dal.Config.CarSpeed = configuration.CarSpeed;
            configChanged = true;
        }
        if (s_dal.Config.SampleExpirationMinutes != configuration.SampleExpirationMinutes) //stage 4
        {
            s_dal.Config.SampleExpirationMinutes = configuration.SampleExpirationMinutes;
            configChanged = true;
        }
        if (s_dal.Config.MaxDeliveryDurationMinutes != configuration.MaxDeliveryDurationMinutes) //stage 4
        {
            s_dal.Config.MaxDeliveryDurationMinutes = configuration.MaxDeliveryDurationMinutes;
            configChanged = true;
        }
        if (s_dal.Config.BaseDeliveryPrice != configuration.BaseDeliveryPrice) //stage 4
        {
            s_dal.Config.BaseDeliveryPrice = configuration.BaseDeliveryPrice;
            configChanged = true;
        }
        if (s_dal.Config.PricePerKm != configuration.PricePerKm) //stage 4
        {
